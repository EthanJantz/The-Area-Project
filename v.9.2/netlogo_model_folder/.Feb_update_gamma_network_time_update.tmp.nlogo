...
globals[ ;
 ...
 interest_rate ; could represent perception_impacts so that an improved percpetion of area due to development vs negative percpetion  of area due to development -- for this model it shoudl be negative.
...
to social_preference_value ; assign vlaue to socal preference variable the will determien network quality for agents, soical preference value is determined my social-inteisity how postive or negative this relaitoships is
  ask  houses[set social_preference  precision (random-float social_affinity * .01)1]
...
to social_network_assessment ; this is for nested desionc makings
  ask houses[
    set network_quality ((count my-out-links)* (tenure + ticks) ) ] ; create a unique network quality of tenure, tick linke and random value on social_preference scale
end

to set_social_valuation
  set soc_valuation ((count my-out-links) * (tenure + ticks) * social_preference)
end
...
to turtle_initial_attributes ; update valuation to be  intial-valaution
 ; ask houses[set valuation ((mortgage + mean[mortgage]of patches with [empty != true and norfolk_owned != true ]  in-radius  1) / 2)
  ...
  ask houses[set tenure(1 + random 70)] ; 1tenure is randon due to populaiton data showing that under 70 the disitbrution for ages are flat meanign equal values for each one
...
;Step 1 valuation with-out networked effects/linked nieghhors considerations ---- there is an issue with when and where to normalize!!!!!
   ...
   ask houses[
    ...
    ;step 1c  create valuation from  social value and montary valuae funcitonign independantly
         set valuation  ((1 - social_preference) * monetary_quality) + soc_quality  ;create updated network quality impacts, mortgage and interest interaction per iteration
...
to  write_header
  file-open(word active_file)
   file-write csv:to-row
   (list
     "ticks" "mortgage-buyout-ratio" "soc_quality" "monetary_valuation" "non-normalized-norfolk-offer" "success-flag""success-total" "houses" "Houses_network_4_plus" "Houses_network_3" "Houses_network_2" "Total_Links" "hold_out_ratio" "initial_val" "neigh_iso" "link_iso"
     )
end
...
@#$#@#$#@
<experiments>
  <experiment name="The Area" repetitions="4" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="15"/>
  ...
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="social_intensity">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="resident_strategy">
      <value value="&quot;Nested&quot;"/>
  ...
  <experiment name="Left_skew_strategies" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="Low density skew" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="blocksize">
  ...
  <experiment name="Right_skewed_model" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <enumeratedValueSet variable="negative_impact">
      <value value="10"/>
  ...
  <experiment name="sens_test_feb_15_nested" repetitions="15" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <enumeratedValueSet variable="blocksize">
      <value value="25"/>
